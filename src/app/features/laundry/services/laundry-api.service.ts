...
export class LaundryApiService {
  ...
  transition(id: string, status: string, notes?: string, tokensDelivered?: number): Observable<unknown> { return this.http.patch<ApiResponse<unknown>>(`${this.baseUrl}/usages/${id}/status`, { status, notes: notes || null, tokensDelivered: tokensDelivered ?? null }).pipe(map((response) => response.data)); }
}
