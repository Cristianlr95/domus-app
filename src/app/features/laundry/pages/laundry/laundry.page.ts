...
export class LaundryPage {
  ...
  cancel(usage: LaundryUsage): void { if (this.canManage || this.mutating || !['REQUESTED', 'AUTHORIZED'].includes(usage.status)) return; this.submitTransition(usage.id, 'CANCELLED'); }
